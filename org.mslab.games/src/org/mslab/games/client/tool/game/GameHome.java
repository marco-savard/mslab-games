...
import org.mslab.games.client.tool.game.queens.QueensTheme;
...
public class GameHome extends GridPanel implements ClickHandler {
	...
	private GameButton _queens, _solitaire; 
	...
	public GameHome(GameShell owner) {
		...
		_solitaire = new GameButton(icon, "Solitaire"); 
		_solitaire.addClickHandler(this);
		_grid.setWidget(row, col, _solitaire);
		col++;
		
	     icon = new HTML("&#9813;");
		_queens = new GameButton(icon, "8 reines"); 
	...
	class GameButton extends GridPanel implements MouseOverHandler, MouseOutHandler {
		GameButton(Widget icon, String titleText) {
